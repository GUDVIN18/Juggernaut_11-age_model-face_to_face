...
"""
from AI_models.inswapper.restoration import *
import os
import cv2
import copy
import argparse
...
def log_success(message):
    print(f'\033[92m{message}\033[0m')  # ANSI escape code for green text
...
def start(source_img, target_img, output_img, upscale, codeformer_fidelity,
          source_indexes="-1", target_indexes="-1",face_restore=True, 
          background_enhance=True, face_upsample=True):
    ...
    try:
        ...
        from server_1.settings import BASE_DIR
        ...
        log_success(f'Result saved successfully: {output_img}')
        return True

    except Exception as e:
        print(f'\n\n\nОШИБКА ВНУТРИ start: Не распознано лицо {e}\n\n\n' )
        return False
        # data = {'face':'no',}
        # return data