...
import os
import django
from generation_target_img import generate_image
...
def process_task(process_id):
    ...
    from inswapper.swapper import start
    ...
    try:
        ...
        target_img = generate_image(process.prompt)
        process.save()
        ...
        url = f"http://62.68.146.176:8585/finish_task_status"
        print(f'\n\n---- STATUS START {status}----\n\n')
        if status == False:
            data = {
    ...
    while True:
        try:
            ...
            for process in unstarted_processes:
                ...
                age = source_age(process.source_img.path)
                if age:
                    process.prompt = process.prompt.replace('**', age)
                    process.save()
